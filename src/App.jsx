    ...
    return (
      <div style={{ display: "grid", gap: 12 }}>
        ...
        >
          ...
          {dashboardCard(
            "Hızlı işlemler",
            "Sık kullandığın alanlara kısa yoldan geç.",
            <div
              style={{
                display: "grid",
            ...
            >
              {quickButton("Yeni müşteri", "customers", "👤", true)}
              {quickButton("Yeni portföy", "portfolios", "🏢")}
              {quickButton("Randevular", "appointments", "📅")}
              {quickButton("Sözleşmeler", "contracts", "📄")}
              {quickButton("Kira sözleşmesi", "rentalContracts", "🏘️")}
              {quickButton("Raporlar", "reports", "📊")}
            </div>,
            { alignSelf: "stretch" }
          )}
        </div>
        ...
        >
          <div style={{ display: "grid", gap: 12 }}>
            {dashboardCard(
              "Sözleşme uyarıları",
              "Yaklaşan ve kritik sözleşmeleri öne çıkarır.",
              criticalContracts.length === 0 ? (
                renderEmpty("Kritik sözleşme yok", "Şu anda acil takip gerektiren sözleşme görünmüyor.")
              ) : (
                <div style={{ display: "grid", gap: 8 }}>
                  {criticalContracts.slice(0, 3).map((item) => (
                    <div
                      key={item.id}
                      style={{
                        background: colors.panel2,
                    ...
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "flex-start", flexWrap: "wrap" }}>
                        <div>
                          <div style={{ fontWeight: 900, fontSize: 14 }}>{item.contractNo || "Sözleşme"}</div>
                          <div style={{ color: colors.sub, marginTop: 4, fontSize: 12.5 }}>{item.owner || "-"}</div>
                          <div style={{ color: colors.sub, marginTop: 2, fontSize: 12.5 }}>Bitiş: {formatShortDate(item.endDate)}</div>
                        </div>
            ...
            {dashboardCard(
              ...
              "Hareket alan portföyleri kısa özetle gör.",
              highAttentionPortfolios.length === 0 ? (
                renderEmpty("Portföy yok", "İlk portföylerini eklediğinde burada öne çıkanlar listelenecek.")
              ) : (
                <div style={{ display: "grid", gap: 8 }}>
                  {highAttentionPortfolios.map((item) => (
                    <button
                      key={item.id}
                    ...
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                        <div style={{ minWidth: 0, flex: 1 }}>
                          <div style={{ fontWeight: 900, fontSize: 14 }}>{item.title || "İsimsiz portföy"}</div>
                          <div style={{ color: colors.sub, marginTop: 4, fontSize: 12.5 }}>
                            {item.city || "-"} / {item.district || "-"} • {formatCurrency(item.price)}
                          </div>
                        </div>
                        <span style={statusBadge(item.status || "aktif")}>{item.status || "aktif"}</span>
                      </div>
                      ...
                      >
                        <div style={{ background: colors.panel3, border: `1px solid ${colors.border}`, borderRadius: 12, padding: 8 }}>
                          <div style={{ fontSize: 10.5, color: colors.sub }}>Eşleşme</div>
                          <div style={{ fontWeight: 900, marginTop: 3, fontSize: 13.5 }}>{item.matchCount}</div>
                        </div>
                        <div style={{ background: colors.panel3, border: `1px solid ${colors.border}`, borderRadius: 12, padding: 8 }}>
                          <div style={{ fontSize: 10.5, color: colors.sub }}>Randevu</div>
                          <div style={{ fontWeight: 900, marginTop: 3, fontSize: 13.5 }}>{item.appointmentCount}</div>
                        </div>
                        <div style={{ background: colors.panel3, border: `1px solid ${colors.border}`, borderRadius: 12, padding: 8 }}>
                          <div style={{ fontSize: 10.5, color: colors.sub }}>Gün</div>
                          <div style={{ fontWeight: 900, marginTop: 3, fontSize: 13.5 }}>{item.daysActive}</div>
                        </div>
                        <div style={{ background: colors.panel3, border: `1px solid ${colors.border}`, borderRadius: 12, padding: 8 }}>
                          <div style={{ fontSize: 10.5, color: colors.sub }}>No</div>
                          <div style={{ fontWeight: 900, marginTop: 3, fontSize: 13.5 }}>{item.portfolioNo || "-"}</div>
                        </div>
                      </div>
                    </button>
                  ))}
          ...
          <div style={{ display: "grid", gap: 12 }}>
            {dashboardCard(
              "Haftalık takvim ve portföy durumu",
              "Hafta görünümü ve envanter dağılımı tek alanda.",
              <div style={{ display: "grid", gap: 12 }}>
                <div
                  style={{
                    display: "grid",
                ...
                >
                  {weekAppointments.map((item) => (
                    <div
                      key={item.iso}
                      style={{
                        background: item.isToday ? colors.primarySoft : colors.panel2,
                        border: `1px solid ${item.isToday ? colors.primary : colors.border}`,
                        borderRadius: 14,
                        padding: 10,
                        textAlign: "center",
                      }}
                    >
                      <div style={{ fontWeight: 900, fontSize: 11.5, textTransform: "capitalize" }}>{item.label}</div>
                      <div style={{ color: colors.sub, fontSize: 11, marginTop: 3 }}>{item.shortDay}</div>
                      <div style={{ fontWeight: 900, fontSize: 19, marginTop: 6, lineHeight: 1 }}>{item.count}</div>
                    </div>
                  ))}
                </div>
                ...
                >
                  {portfolioStatusSummary.map((item) => (
                    <div
                      key={item.label}
                      style={{
                        background: colors.panel2,
                    ...
                    >
                      <div style={{ color: colors.sub, fontSize: 11.5 }}>{item.label}</div>
                      <strong style={{ fontSize: 18, display: "block", marginTop: 5 }}>{item.value}</strong>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {dashboardCard(
              "Son müşteriler",
              "Yeni eklenen veya güncellenen kişiler.",
              recentCustomers.length === 0 ? (
                renderEmpty("Müşteri kaydı yok", "İlk müşteriyi eklediğinde burada listelenecek.")
              ) : (
                <div style={{ display: "grid", gap: 8 }}>
                  {recentCustomers.map((item) => (
                    ...
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center" }}>
                        <div style={{ minWidth: 0, flex: 1 }}>
                          <div style={{ fontWeight: 900, fontSize: 14 }}>{item.fullName || "İsimsiz müşteri"}</div>
                          <div style={{ color: colors.sub, marginTop: 4, fontSize: 12.5, lineHeight: 1.5 }}>
                            {item.phone || "Telefon yok"}
                            {item.interestedLocation ? ` • ${item.interestedLocation}` : ""}
                          </div>
                        </div>
                        <span style={statusBadge(item.customerType || "alıcı")}>{item.customerType || "alıcı"}</span>
                      </div>
                      <div style={{ color: colors.sub, marginTop: 6, fontSize: 12 }}>
                        Bütçe: {formatCurrency(item.budgetMin) || "-"} / {formatCurrency(item.budgetMax) || "-"}
                      </div>
                    </button>
                  ))}
                </div>
